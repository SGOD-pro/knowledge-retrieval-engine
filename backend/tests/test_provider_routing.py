...
from providers.bedrock_models import (
    get_embedding_model,
    get_llm_model,
    get_reranker_model,
)
from providers.provider_client import ConfigurationError, get_active_provider
...
def test_r29_no_dev_in_prod():
    ...
    with pytest.raises(
        ConfigurationError, match="MODEL_PROVIDER=dev is strictly prohibited in prod"
    ):
        get_active_provider()
