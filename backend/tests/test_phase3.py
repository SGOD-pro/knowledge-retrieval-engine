...
from kre.retrieval.fidelity_check import CoverageError

@pytest.fixture
def mock_llm():
    with patch("kre.graph.langgraph_pipeline.call_llm") as mock_call:
        mock_call.return_value = {"answer": "MOCK_ANSWER", "citations": ["c1"]}
        yield mock_call

def test_r02_planner_zero_llm_calls():
    # Calling planner directly
    with patch("kre.llm.llm_service.call") as mock_llm:
...
def test_r02_full_path_exactly_one_llm_call(mock_llm):
    ...
    with patch("kre.retrieval.graph_retriever.GraphRetriever.expand") as mock_graph:
        with patch("kre.graph.langgraph_pipeline.check_fidelity") as mock_fidelity:
...
def test_r10_all_modules_log_required_fields(caplog):
    ...
    with patch("kre.graph.langgraph_pipeline.call_llm") as mock_llm:
...
def test_r18_nova_micro_zero_query_time_calls():
    with patch("kre.ingestion.concept_service.extract_properties_nova_micro") as mock_nova:
        with patch("kre.graph.langgraph_pipeline.call_llm") as mock_llm:
