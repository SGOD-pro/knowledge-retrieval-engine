...
from retrieval.okf_retriever import OKFRetriever
from graph.langgraph_pipeline import pipeline
from retrieval.fidelity_check import CoverageError

@pytest.fixture
def mock_llm():
    with patch("graph.langgraph_pipeline.call_llm") as mock_call:
        mock_call.return_value = {"answer": "MOCK_ANSWER", "citations": ["c1"]}
        yield mock_call

def test_r02_planner_zero_llm_calls():
    # Calling planner directly
    with patch("llm.llm_service.call") as mock_llm:
...
def test_r02_full_path_exactly_one_llm_call(mock_llm):
    ...
    with patch("retrieval.graph_retriever.GraphRetriever.expand") as mock_graph:
        with patch("graph.langgraph_pipeline.check_fidelity") as mock_fidelity:
...
def test_r10_all_modules_log_required_fields(caplog):
    ...
    with patch("graph.langgraph_pipeline.call_llm") as mock_llm:
        mock_llm.return_value = {"answer": "MOCK_ANSWER", "citations": []}
        with patch("retrieval.graph_retriever.GraphRetriever.expand") as mock_graph:
            with patch("graph.langgraph_pipeline.check_fidelity") as mock_fidelity:
                with patch("graph.langgraph_pipeline.rerank") as mock_rerank:
                    from models import Chunk
                    dummy_chunk = Chunk(id="c1", document_id="d1", source_format="pdf", text="revenue decreased sample text", element_type="text", page_number=1)
                    mock_rerank.return_value = [dummy_chunk]
                    mock_graph.return_value = [dummy_chunk]
                    pipeline.run("Why did revenue decrease?")
...
def test_r18_nova_micro_zero_query_time_calls():
    from ingestion_lambda import concept_service
    with patch.object(concept_service, "extract_properties_nova_micro") as mock_nova:
        with patch("graph.langgraph_pipeline.call_llm") as mock_llm:
