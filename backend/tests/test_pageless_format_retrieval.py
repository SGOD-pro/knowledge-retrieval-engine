...
def test_pageless_chunks_preserved_in_page_index_narrowing():
    ...
    c_docx = _make_chunk("docx_c1", "DOCX content without page number", page_number=None, format="docx")
    
    retriever = PageIndexRetriever()
    with patch("src.services.retrieval.page_index_retriever.structural_score", return_value=1.0):
        selected, pages, chunk_ids = retriever.filter_and_rank("query", [c_pdf, c_docx], top_k=10)
...
def test_vector_search_includes_pageless_chunks_when_pages_present():
    ...
    c_pdf = _make_chunk("pdf_c1", "PDF content on page 5", page_number=5, format="pdf")
    c_docx = _make_chunk("docx_c1", "DOCX content without page number", page_number=None, format="docx")
    c_irrelevant = _make_chunk("pdf_c2", "PDF content on page 99", page_number=99, format="pdf")
    ...
    assert "docx_c1" in result_ids, "DOCX pageless chunk in candidate_chunk_ids must be found"
    assert "pdf_c2" not in result_ids, "Irrelevant PDF chunk on page 99 must be excluded"
