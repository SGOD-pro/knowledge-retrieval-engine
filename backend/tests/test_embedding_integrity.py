...
def _make_sample_chunk(cid="c1", text="Sample chunk text", f_emb=None, full_emb=None, page_number=None):
    return Chunk(
        id=cid,
        document_id="doc1",
        source_format="docx",
...
def test_embed_chunks_dual_fails_loudly_on_api_error():
    ...
    with patch("providers.embedding_provider.embed_batch", side_effect=RuntimeError("Bedrock Titan API Timeout")):
...
def test_embed_batch_rejects_zero_padding():
    ...
    with patch("providers.embedding_provider.embed_text", side_effect=RuntimeError("API down")):
...
def test_cloud_repository_save_rejects_zero_padded_embeddings():
    ...
    with pytest.raises((ValueError, RuntimeError), match="[Ii]ntegrity|embedding|zero-padded"):
        repo.save(doc)
...
def test_cloud_repository_save_rejects_dimension_mismatch():
    ...
    with pytest.raises((ValueError, RuntimeError), match="[Ii]ntegrity|embedding|dimension"):
        repo.save(doc)
