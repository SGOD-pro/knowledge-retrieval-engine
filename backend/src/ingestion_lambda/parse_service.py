...
def parse_file(path: Path, document_id: str | None = None) -> Document:
    ...
    logger.info(
        ...
        document_id, source_format, len(chunks), latency_ms,
    )
    return doc


def ingest_document(path: Path, document_id: str | None = None, provider: str = "dev") -> Document:
    ...
    try:
        ...
        logger.info(
            ...
            doc.id, (time.perf_counter() - t0) * 1000,
        )
