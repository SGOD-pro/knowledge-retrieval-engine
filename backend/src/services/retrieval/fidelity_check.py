...
def check_fidelity(query: str, context_chunks: list[str]) -> float:
    ...
    logger.info("fidelity_check.latency_ms=%.2f fidelity_check.confidence_score=%.2f", latency_ms, max_sim)
    
    threshold = settings.FIDELITY_THRESHOLD
    if max_sim < threshold:
        raise CoverageError(f"Coverage ratio {max_sim:.2f} is below {threshold} threshold.")
