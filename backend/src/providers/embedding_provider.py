...
def embed_text(text: str, provider: str | None = None, max_retries: int = 5) -> list[float]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "ThrottlingException" in str(e) or "Too many requests" in str(e):
                ...
                logger.warning("embed_text: Throttled (attempt %d/%d), sleeping %.1fs...", attempt + 1, max_retries, wait_time)
    ...
    raise RuntimeError(f"API embedding failed: {last_err}. Check rate limits or API key.")
...
def embed_batch(texts: Sequence[str], provider: str | None = None, max_retries: int = 3) -> list[list[float]]:
    from config import settings
    if provider == "test" or (provider is None and settings.ENVIRONMENT == "test"):
    ...
    for t in texts:
        ...
        for attempt in range(max_retries):
            try:
                ...
                last_err = e
                wait = (2 ** attempt) * 0.5
                logger.warning("embed_batch: retry %d/%d for text after %.2fs error=%s", attempt + 1, max_retries, wait, e)
                time.sleep(wait)
        else:
