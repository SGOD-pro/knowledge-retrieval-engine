...
class CloudRepository:
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            self.okf_entities_table.meta.client.get_waiter("table_exists").wait(
                TableName="okf_entities"
            )
        except Exception as e:
            if "ResourceInUseException" not in str(e):
        ...
        try:
            ...
            self.okf_properties_table.meta.client.get_waiter("table_exists").wait(
                TableName="okf_properties"
            )
        except Exception as e:
            if "ResourceInUseException" not in str(e):
    ...
    def save(self, document: Document) -> None:
        ...
        for chunk in document.chunks:
            ...
            if all(x == 0.0 for x in chunk.embedding_fast):
                raise ValueError(
                    f"DataIntegrityError: Chunk {chunk.id} has all-zero embedding_fast"
                )
            if all(x == 0.0 for x in chunk.embedding_full):
                raise ValueError(
                    f"DataIntegrityError: Chunk {chunk.id} has all-zero embedding_full"
                )
            if all(x == 0.0 for x in chunk.embedding_full[384:]):
                raise ValueError(
                    f"DataIntegrityError: Chunk {chunk.id} has zero-padded embedding_full (dims 384:1024 are all zero)"
                )
                ...
                for i in range(0, len(points), batch_size):
                    ...
                    for attempt in range(3):
                        try:
                            ...
                            if (
                                "429" in str(qe)
                                or "Too Many Requests" in str(qe)
                                or "timed out" in str(qe).lower()
                            ):
                                logger.warning(
                                    "Qdrant rate limit/timeout hit (attempt %d/3). Sleeping 5s...",
                                    attempt + 1,
                                )
                                time.sleep(5)
                            else:
    ...
    def search_vector(
        ...
        if settings.ENVIRONMENT == "test":
            ...
            has_page_constraint = (
                candidate_page_ids is not None and len(candidate_page_ids) > 0
            )
            has_chunk_constraint = (
                candidate_chunk_ids is not None and len(candidate_chunk_ids) > 0
            )
            ...
            for chunk in _IN_MEMORY_CHUNKS.values():
                ...
                if has_page_constraint or has_chunk_constraint:
                    page_match = (
                        has_page_constraint and chunk.page_number in candidate_page_ids
                    )
                    chunk_match = (
                        has_chunk_constraint and str(chunk.id) in candidate_chunk_ids
                    )
                    if not (page_match or chunk_match):
                        continue
                c_vec = (
                    chunk.embedding_fast
                    if embedding_column == "embedding_fast"
                    else chunk.embedding_full
                )
                if c_vec:
    ...
    def get_okf_properties(self, entities: list[str]) -> list[dict]:
        ...
        for entity in entities:
            try:
                ...
                logger.warning(
                    "DynamoDB OKF property lookup failed for %s: %s", entity, e
                )
        return results
    ...
    def expand_graph(self, start_entities: list[str], max_hops: int = 2) -> list[dict]:
        ...
        while queue and len(results) < 40:
            ...
            try:
                ...
                logger.warning(
                    "DynamoDB OKF graph traversal failed for %s: %s", current_entity, e
                )
